...
/**
 * Passcode component for secure login with a 6-digit code
 * @returns {React.ReactElement}
 */
function Passcode() {
    // Constants
    const CORRECT_PASSCODE = '010523' // Anniversary date format
    ...
    /**
     * Handles number button clicks and validates passcode
     * @param {number} number - The number clicked on the keypad
     */
    const handleNumberClick = (number) => {
        if (passcode.length < 6) {
            ...
            // Check the passcode when 6 digits are entered
            if (newPasscode.length === 6) {
                ...
                if (enteredPasscode === CORRECT_PASSCODE) {
                    ...
                    // Error flow
                    setMessage('Hayolo salah, hint: our anniversary date!')
                    setTimeout(() => {
                        setPasscode([])
                        setMessage('')
                    }, 4000)
    ...
    /**
     * Clears the entered passcode and any displayed message
     */
    const handleCancel = () => {
        setPasscode([])
        setMessage('')
    ...
    // Keypad numbers array for mapping
    const keypadNumbers = [
        { num: 1 }, { num: 2 }, { num: 3 },
        { num: 4 }, { num: 5 }, { num: 6 },
        { num: 7 }, { num: 8 }, { num: 9 }
        ...
        <div className="min-h-screen w-full bg-black/60 backdrop-blur-sm text-white flex items-center justify-center overflow-hidden">
            <div className="flex flex-col items-center max-w-full">
                ...
                {message && (
                    <div
                        className={`mb-4 -mt-9 text-sm font-bold ${
                            message === 'Anjayy!! :)' ? 'text-green-500' : 'text-red-500'
                        }`}
                    >
                        {message}
                    </div>
                ...
                <div className="grid grid-cols-3 gap-4 mb-8 max-w-full">
                    ...
                    {/* Zero Button (centered in bottom row) */}
